...
// 其他縣市的公車路網（TDX，只有路線與站牌、沒有免費即時車輛）：一個縣市 3 次呼叫、約 10～20 MB
// 可用縣市：Taoyuan Taichung Tainan Kaohsiung Keelung Hsinchu HsinchuCounty MiaoliCounty ChanghuaCounty NantouCounty YunlinCounty ChiayiCounty Chiayi PingtungCounty YilanCounty HualienCounty TaitungCounty KinmenCounty PenghuCounty LienchiangCounty
const BUS_TDX_CITIES = (process.env.BUS_TDX_CITIES ?? 'Taichung').split(',').map(s => s.trim()).filter(Boolean);
...
// TDX 版公車路網 → 與臺北公開檔相同的檔案格式
async function fetchTdxBusCity(token, city, index, stats) {
  const dir = path.join(OUT, 'bus', city); await fs.mkdir(path.join(dir, 'route'), { recursive: true });
  const get = async kind => { const res = await tdxGet(token, `${API_BASE}Bus/${kind}/City/${city}?$format=JSON&$top=100000`, `bus/${city}/${kind}`, stats); if (!res.ok) throw new Error(`${kind} HTTP ${res.status}`); const j = firstArray(await res.json()); await sleep(DELAY_MS); return j; };
  const routes = await get('Route'), stopOfRoute = await get('StopOfRoute'), shapes = await get('Shape').catch(() => []);
  const zh = v => (v && typeof v === 'object') ? (v.Zh_tw || v.En || '') : (v || '');
  const list = [];
  for (const r of routes) {
    const subs = (r.SubRoutes && r.SubRoutes.length) ? r.SubRoutes : [{ SubRouteUID: r.RouteUID, SubRouteID: r.RouteID, SubRouteName: r.RouteName }];
    for (const sr of subs) list.push({ id: String(r.RouteUID || r.RouteID), sub: String(sr.SubRouteUID || sr.SubRouteID), name: zh(r.RouteName) || String(r.RouteID), subName: zh(sr.SubRouteName) !== zh(r.RouteName) ? zh(sr.SubRouteName) : '', from: r.DepartureStopNameZh || '', to: r.DestinationStopNameZh || '' });
  }
  await fs.writeFile(path.join(dir, 'routes.json'), JSON.stringify(list));
  const byRoute = new Map(); const ensure = rid => { if (!byRoute.has(rid)) byRoute.set(rid, { stops: { 0: [], 1: [] }, shape: {} }); return byRoute.get(rid); };
  for (const sr of stopOfRoute) {
    const rid = String(sr.RouteUID || sr.RouteID), gb = String(sr.Direction) === '1' ? 1 : 0, isMain = !sr.SubRouteUID || sr.SubRouteUID === sr.RouteUID || sr.SubRouteID === sr.RouteID;
    const r = ensure(rid); if (r.stops[gb].length && !isMain) continue;                          // 主線優先
    r.stops[gb] = (sr.Stops || []).map(st => ({ id: String(st.StopUID || st.StopID), name: zh(st.StopName), seq: Number(st.StopSequence) || 0, lat: Number(st.StopPosition && st.StopPosition.PositionLat), lon: Number(st.StopPosition && st.StopPosition.PositionLon) })).sort((a, b) => a.seq - b.seq);
  }
  for (const sh of shapes) {
    const rid = String(sh.RouteUID || sh.RouteID), gb = String(sh.Direction) === '1' ? 1 : 0, isMain = !sh.SubRouteUID || sh.SubRouteUID === sh.RouteUID || sh.SubRouteID === sh.RouteID;
    const r = byRoute.get(rid); if (!r || !sh.Geometry) continue;
    if (!r.shape[gb] || isMain) r.shape[gb] = sh.Geometry;
  }
  let files = 0;
  for (const [rid, r] of byRoute) { await fs.writeFile(path.join(dir, 'route', rid.replace(/[^\w.-]/g, '_') + '.json'), JSON.stringify(r)); files++; }
  index[city] = { source: 'tdx', routes: list.length, stops: stopOfRoute.length, shapes: shapes.length, files };
  console.log(`  公車 ${city}（TDX）：${list.length} 條路線、${files} 個路線檔`);
}
...
async function main() {
  ...
  if (!SYSTEMS.length && !RAIL.length && !BUS_TDX_CITIES.length) { await fs.writeFile(path.join(OUT, 'index.json'), JSON.stringify(index, null, 1)); return; }
  ...
  for (const city of BUS_TDX_CITIES) await fetchTdxBusCity(token, city, index.bus, stats).catch(e => { stats.failed++; index.bus[city] = 'error: ' + e.message.slice(0, 80); console.log(`  公車 ${city}（TDX）失敗，保留舊檔 — ${e.message}`); });
